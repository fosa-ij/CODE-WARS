...
const catShht = (str) => {
    const indexCat = str.indexOf('c')
    const noCatShtt = str.substr(indexCat +1).replaceAll('Eek!', '')
    const correctOrder = noCatShtt
    .split(' ')
    .reverse().map(x => x.split('').reverse().join(''))
    .join(' ')
    return correctOrder
    //* "Split this string up on *lowercase* 'V'. The first character in each substring besides this first one is what you need to remember for tomorrow's task." === "Space"
} 
console.log(catShht(test));